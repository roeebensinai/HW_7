...
//------------------------------------------
struct grades{
	struct list* student_list; // pointer to the student's nodes list
};
//------------------------------------------
struct student_node{ 
	int student_id;
	char *student_name; 
	struct list* courses_list; // pointer to the student's nodes course list
};
//-----------------------------------------------------------------
struct courses_node{
	char *course_name; 
...
//-------------------------------------------------------------------------
// function that finds the student and returns pointer to node
...
int student_node_clone(void *element, void **output){
	...
	*output_casted = (struct student_node*)malloc(sizeof(struct student_node));
	...
	//clone name
	char *temp_name = (char*)malloc(sizeof(char)*(strlen(element_as_node->student_name)+1));
...
int course_node_clone(void *element, void **output){
	...
	*output1_casted = (struct courses_node*)malloc(sizeof(struct courses_node));
	...
	//clone name
	char *temp_name = (char*)malloc(sizeof(char)*(strlen(element_as_node->course_name)+1));
...
void student_node_destroy(void *element){
	...
	list_destroy(element_as_node->courses_list); //destroy all student's course list
	free(element_as_node);
}
//--------------------------------------------------------------------
...
int grades_add_student(struct grades *grades, const char *name, int id){
	...
	struct student_node* new_node = create_student_node((char*)name,id);
		...
		for (int i=0; i<list_len ;i++){      // run for all students
			...
			if(id==(curren_node_ptr->student_id)){     //we found the student
				if((curren_node_ptr->courses_list) == NULL){     //enter if the student does not have a courses list
					...
					struct courses_node* new_node = create_courses_node((char*)name,grade);
...
float grades_calc_avg(struct grades *grades, int id, char **out){
	...
	char *temp_name = (char*)malloc(sizeof(char)*(strlen(student->student_name)+1)); //clone the student's name:
